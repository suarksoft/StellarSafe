/**
 * StellarSafe SDK - TypeScript/JavaScript Client
 * 
 * @example
 * ```typescript
 ...
 *   apiKey: 'sk_test_xxx',
 *   baseUrl: 'https://api.stellarsafe.io'
 * });
 * 
...
export interface StellarSafeConfig {
  apiKey: string;
  baseUrl?: string;
  timeout?: number;
}
...
export class StellarSafeError extends Error {
  constructor(
    message: string,
    public code: string,
    ...
    super(message);
    this.name = 'StellarSafeError';
  }
}

export class StellarSafeClient {
  private apiKey: string;
  private baseUrl: string;
  private timeout: number;

  constructor(config: StellarSafeConfig) {
    this.apiKey = config.apiKey;
    this.baseUrl = config.baseUrl || 'https://api.stellarsafe.io';
 ...
 * ```typescript
 * function SendModal() {
 *   const { analysis, loading, error } = useStellarSafeAnalysis(
 *     destination,
 *     'public'
 *   );
 *   
...
export default StellarSafeClient;
